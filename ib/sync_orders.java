...
{
	public static final String TYPE_PLACE = types.SYNC_ORDERS_PLACE;
	public static final String TYPE_PLACE_MARKET = types.SYNC_ORDERS_PLACE_MARKET;
	public static final String TYPE_PLACE_STOP = types.SYNC_ORDERS_PLACE_STOP;
	public static final String TYPE_PLACE_LIMIT = types.SYNC_ORDERS_PLACE_LIMIT;
	public static final String TYPE_UPDATE = types.SYNC_ORDERS_UPDATE;
	public static final String TYPE_UPDATE_START_VALUE = types.SYNC_ORDERS_UPDATE_START_VALUE;
	public static final String TYPE_UPDATE_START_MARKET = types.SYNC_ORDERS_UPDATE_START_MARKET;
	public static final String TYPE_UPDATE_STOP_VALUE = types.SYNC_ORDERS_UPDATE_STOP_VALUE;
	public static final String TYPE_UPDATE_STOP_MARKET = types.SYNC_ORDERS_UPDATE_STOP_MARKET;
	...
	public static String check_place(String type_) { return accessory.types.check_type(type_, TYPE_PLACE); }
	
	public static String check_update(String type_) { return accessory.types.check_type(type_, TYPE_UPDATE); }
	...
	private static boolean place_update(order order_, String update_type_, double update_val_) 
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			Order order = get_order_ib(order_, id, parent, update_type, update_val_, is_sec);
		...
		{
			if (update_type.equals(TYPE_UPDATE_START_VALUE)) update_order(id, update_val_, true); 
			else if (update_type.equals(TYPE_UPDATE_STOP_VALUE)) update_order(id, update_val_, false); 			
		}
	...
	private static Order get_order_ib(order order_, int id_, int parent_, String update_type_, double update_val_, boolean is_last_)
	{
		...
		boolean is_market = order_.get_type().equals(TYPE_PLACE_MARKET);
		boolean is_update = strings.is_ok(update_type_);
		...
		if (!is_main || (is_update && is_market)) action = ACTION_SELL;
		order.action(action);
		...
		if (is_update)
		{
			...
			boolean start_market = strings.are_equal(update_type_, TYPE_UPDATE_START_MARKET);
			boolean stop_market = strings.are_equal(update_type_, TYPE_UPDATE_STOP_MARKET);
			boolean start_value = strings.are_equal(update_type_, TYPE_UPDATE_START_VALUE);
			boolean stop_value = strings.are_equal(update_type_, TYPE_UPDATE_STOP_VALUE);
		...
		if (is_market && (is_main || is_update)) order.orderType(TYPE2_MARKET);
		else
		{
			if (val == WRONG_VALUE) return null;
			
			String type = TYPE2_STOP;
			if (is_main && order_.get_type().equals(TYPE_PLACE_LIMIT)) type = TYPE2_LIMIT;
