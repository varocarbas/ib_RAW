...
abstract class remote_execute
{
	...
	private static boolean execute(HashMap<String, String> vals_)
	{
		...
		else
		{
			...
			{
				executed = cancel(request, order_id);
	...
	{ 
		...
		order order = sync_orders.get_order(order_id_, false);
	...
	private static boolean cancel(int request_, int order_id_) 
	{
		boolean is_ok = false;
		
		is_ok = orders.cancel(order_id_);
